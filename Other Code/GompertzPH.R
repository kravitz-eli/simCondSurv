#########################################################################################################
#########################################################################################################
##Title: Gompertz Projection of Overall Survival in Support of RELAY Submission
...
update(mod,10000)
out.mod<- coda.samples(mod,variable.names=c('a','b','P','beta'),n.iter=100000)
...
for(i in 1:length(s)){
  mE[i] = median(exp(-(out$a/out$b)*(exp(out$b*s[i])-1)))
  mR[i] = median(exp(-(out$a/out$b)*(exp(out$b*s[i])-1))^exp(out$beta))
}
...
#   1/b*log(b/a*exp(-b*t0)*u*beta^-1 + 1)
# }

InvCondCDF = function(u, t0, a, b, beta){
  # 1 / b * log( exp(b * t0) - b / (beta * a) * log(1 - u)) - t0

  # 1/b * log(1 - (b * exp(-b * t0) * log((1 - u)^(1 / beta)) )/ a )
...
for(i in 1:nrow(out)){
  ...
  times[TRT[R]==1,i] = InvCondCDF(a[TRT[R]==1],t[R][TRT[R]==1],out$a[i],out$b[i],1)
  times[TRT[R]==2,i] = InvCondCDF(a[TRT[R]==2],t[R][TRT[R]==2],out$a[i],out$b[i],exp(out$beta[i]))
}
...
median(mP[is.na(mP)==F])
quantile(mP[is.na(mP)==F],c(.025,.975))
