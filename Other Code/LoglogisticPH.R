#########################################################################################################
#########################################################################################################
##Title: Log-Logistic (Proportional Hazards) Projection of Overall Survival in Support of RELAY Submission
...
model{
  ...
  for(i in 1:N){
    ...
    h[i] <- exp(TRT[i]*eta)*(beta/alpha)*pow(t[i]/alpha,beta-1)/(1+pow(t[i]/alpha,beta))
    S[i] <- pow(1/(1+pow(t[i]/alpha,beta)),exp(TRT[i]*eta))
...
update(mod,10000)
out.mod<- coda.samples(mod,variable.names=c('alpha','beta','eta','P'),n.iter=100000)
...
for(i in 1:length(s)){
  mE[i] = median(1/(1+(s[i]/out$alpha)^out$beta))
  mR[i] =  median((1/(1+(s[i]/out$alpha)^out$beta))^exp(out$eta))
}
...
median(out$alpha)
median(out$alpha*(2^(1/exp(out$eta))-1)^(1/out$beta))
...
for(i in 1:nrow(out)){
  ...
  times[TRT[R]==1,i] = InvCondCDF(a[TRT[R]==1],t[R][TRT[R]==1],out[i,2],out[i,3],1)
  times[TRT[R]==2,i] = InvCondCDF(a[TRT[R]==2],t[R][TRT[R]==2],out[i,2],out[i,3],exp(out[i,4]))
}
