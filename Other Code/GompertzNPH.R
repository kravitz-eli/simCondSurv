...
update(mod,1000)
# out.mod<- coda.samples(mod,variable.names=c('a','b','P'),n.iter=100000)
out.mod<- coda.samples(mod,variable.names=c('a','b', 'P'),n.iter=100000)
...
for(i in 1:length(s)){
  mE[i] = median(exp(-(out[,2]/out[,4])*(exp(out[,4]*s[i])-1)))
  mR[i] = median(exp(-(out[,3]/out[,5])*(exp(out[,5]*s[i])-1)))
}
...
median((1/out[,4])*log(1+log(2)*(out[,4]/out[,2])))
median((1/out[,5])*log(1+log(2)*(out[,5]/out[,3])))
