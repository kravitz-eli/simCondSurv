#########################################################################################################
#########################################################################################################
##Title: Weibull (Proportional Hazards) Projection of Overall Survival in Support of RELAY Submission
...
dat = read.csv(here("Data", "monaleesa2_os.csv"), header = T)
...
update(mod, 1000)
out.mod<- coda.samples(mod,variable.names=c('lambda','nu','beta','P'),n.iter=5000)
...
for(i in 1:length(s)){
  ...
  mE[i] = median(exp(-out$lambda*s[i]^out$nu))
  
  ##Curve for ram arm
  mR[i] = median(exp(-exp(out$beta)*out$lambda*s[i]^out$nu))
}

# Compare to Zach's original Code
##Posterior distribution for true median PFS on control arm
ME = (-log(.5)/(out$lambda))^(1/out$nu)
median(ME)
quantile(ME,c(.025,.975))

##Posterior distribution for true median PFS on Ram arm
MR = (-log(.5)/(exp(out$beta)*out$lambda))^(1/out$nu)
median(MR)
quantile(MR,c(.025,.975))
...
for(i in 1:nrow(out)){
  ...
  times[TRT[R]==1,i] = InvCondCDF(a[TRT[R]==1],t[R][TRT[R]==1],out$nu[i],out$lambda[i],1)
  times[TRT[R]==2,i] = InvCondCDF(a[TRT[R]==2],t[R][TRT[R]==2],out$nu[i],out$lambda[i],exp(out$beta[i]))
}
